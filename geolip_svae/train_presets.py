"""
geolip_svae.train_config
=========================
Named training-preset registry for the SVAE Unified Trainer.

This module holds every named cfg dict that the trainer can consume
(``python -m geolip_svae.train --preset NAME``). It was extracted from
``geolip_svae.train`` so the loop and the catalog evolve independently:
adding a preset never requires editing train.py, and reading the catalog
no longer pulls in the trainer's heavy imports (torch, torchvision, hf_hub).

Public surface
--------------
    PRESETS : dict[str, dict[str, Any]]  every named preset

The trainer re-imports PRESETS so existing callers
